    ...
    def is_blurry(frame1, frame2, rel_threshold, blur_percentage, grid_size):
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                fft_score_orig = float(FrameAnalyzer.fft_sharpness(cell))
                fft_score_new = float(FrameAnalyzer.fft_sharpness(cell2))
                rel_fft_score = (fft_score_orig - fft_score_new) / max(fft_score_orig, 1e-6)

                ten_score_orig = float(FrameAnalyzer.tenengrad(cell))
                ten_score_new = float(FrameAnalyzer.tenengrad(cell2))
                ...
                if relative_blur > (rel_threshold / 100): # mark blurry if both laplacian and fourier transform values are low
